...
public class ChatView extends VerticalLayout implements MessageListener {
    ...
    private void ChatViewPostConstruct() {
        ...
        rbg.setItems("rest", "soap", "jms", "file");
        ...
        vl1.add(hl, taMessage, btnSend, new Hr(), new H4("Log verstuurde berichten:"));
        vl1.setWidth("40%");
        sl.addToPrimary(vl1);
    ...
    private void handleSendButtonClick(ClickEvent<Button> buttonClickEvent) {
        ...
        if (rbg.getValue() != null && rbg.getValue().equalsIgnoreCase("rest")) {
            ...
            try {
                ...
                if (response != null) {
                    ...
                    vl1.add(new Span(msg.getMessage()));
                }
        ...
        } else if (rbg.getValue() != null && rbg.getValue().equalsIgnoreCase("jms")) {
            sender.sendOnQueue(msg);
            Notification.show("Bericht verstuurd via JMS", 3000, Notification.Position.TOP_CENTER);
            taMessage.setValue("");
            taMessage.focus();

            vl1.add(new Span(msg.getMessage()));
        } else if (rbg.getValue() != null && rbg.getValue().equalsIgnoreCase("file")) {
            try {
                ChatRequest req;
                ...
                File file = new File(input.getAbsolutePath() + File.separator + "Testfile-" + suffix + ".xml");
                ...
                Notification.show("Bericht verstuurd via File", 3000, Notification.Position.TOP_CENTER);
                taMessage.setValue("");
                taMessage.focus();
